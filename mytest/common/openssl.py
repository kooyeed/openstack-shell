...
class BaseCertificateConfigure(object):
    ...
    def __init__(self, conf_obj, mytest_user, mytest_group, **kwargs):
        self.conf_dir = os.path.dirname(conf_obj.ca_certs)
        self.use_mytest_user = mytest_user
        self.use_mytest_group = mytest_group
    ...
    def _make_dirs(self, file_name):
        ...
        if os.geteuid() == 0 and self.use_mytest_group:
            os.chown(dir, -1, self.use_mytest_group)

    def _set_permissions(self, file_name, perms):
        os.chmod(file_name, perms)
        if os.geteuid() == 0:
            os.chown(file_name, self.use_mytest_user or -1,
                     self.use_mytest_group or -1)
...
class ConfigurePKI(BaseCertificateConfigure):
    ...
    def __init__(self, mytest_user, mytest_group):
        super(ConfigurePKI, self).__init__(CONF.signing,
                                           mytest_user, mytest_group)
