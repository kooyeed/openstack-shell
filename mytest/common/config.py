...
gettext.install('test', unicode=1)
...
def configure():
    ...
    register_str('certfile', group='ssl',
                 default="/etc/test/ssl/certs/test.pem")
    register_str('keyfile', group='ssl',
                 default="/etc/test/ssl/private/testkey.pem")
    register_str('ca_certs', group='ssl',
                 default="/etc/test/ssl/certs/ca.pem")
    register_str('ca_key', group='ssl',
                 default="/etc/test/ssl/certs/cakey.pem")
    ...
    register_str(
        'certfile',
        group='signing',
        default="/etc/test/ssl/certs/signing_cert.pem")
    register_str(
        'keyfile',
        group='signing',
        default="/etc/test/ssl/private/signing_key.pem")
    register_str(
        'ca_certs',
        group='signing',
        default="/etc/test/ssl/certs/ca.pem")
    ...
    register_str(
        'driver',
        group='catalog',
        default='test.catalog.backends.sql.Catalog')
    register_str(
        'driver',
        group='identity',
        default='test.identity.backends.sql.Identity')
    register_str(
        'driver',
        group='policy',
        default='test.policy.backends.sql.Policy')
    register_str(
        'driver', group='token', default='test.token.backends.kvs.Token')
    register_str(
        'driver', group='trust', default='test.trust.backends.sql.Trust')
    register_str(
        'driver', group='ec2', default='test.contrib.ec2.backends.kvs.Ec2')
    register_str(
        'driver',
        group='stats',
        default='test.contrib.stats.backends.kvs.Stats')
    ...
    register_str(
        'password', group='auth', default='test.auth.plugins.token.Token')
    register_str(
        'token', group='auth',
        default='test.auth.plugins.password.Password')
